<template>
  <v-app-bar app>
    ...
    <h2 style="margin-right: 8px;">GitScratch</h2>

    <v-tooltip bottom>
      <template v-slot:activator="{ on, attrs }">
        <v-btn icon href="/" v-bind="attrs" v-on="on"><v-icon>mdi-home-outline</v-icon></v-btn>
      </template>
      <span>首页</span>
    </v-tooltip>
    <v-tooltip bottom>
      <template v-slot:activator="{ on, attrs }">
        <v-btn icon href="/explore" v-bind="attrs" v-on="on"><v-icon>mdi-compass-outline</v-icon></v-btn>
      </template>
      <span>发现</span>
    </v-tooltip>
    <v-tooltip bottom>
      <template v-slot:activator="{ on, attrs }">
        <v-btn icon href="/project/editor" v-bind="attrs" v-on="on"><v-icon>mdi-plus</v-icon></v-btn>
      </template>
      <span>创建</span>
    </v-tooltip>
    ...
    <v-dialog v-model="login_dialog" width="400">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="transparent" style="margin-right: 8px;" elevation="0" v-bind="attrs" v-on="on">登录</v-btn>
      </template>
      <v-card>
        ...
        <v-divider></v-divider>
        <v-card-actions>
          <v-btn color="blue" text elevation="0" v-on:click="login_dialog = false; register_dialog = true;">没有帐号</v-btn>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="register_dialog" width="400">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="blue" dark style="margin-right: 8px;" elevation="0" v-bind="attrs" v-on="on">注册</v-btn>
      </template>
      <v-card>
        ...
        <v-divider></v-divider>
        <v-card-actions>
          <v-btn color="blue" text elevation="0" v-on:click="register_dialog = false; login_dialog = true;">已有帐号</v-btn>
          <v-spacer></v-spacer>
